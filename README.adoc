...
Note: In a HAML document, if you use the equals separator (`=`) rather than a greater than equals (`=>`) in a hash key-value assignment, Awestruct will crash.
...
* `authors` (comma separated list of original authors of the guide)
* `translators` (comma separated list, for translations only)
